...
premodel_dir = 'H:/desktop/new_test/tfModel1_1/' #load model from here
...
######deconvolution1 : ... * 2 = x2############
#deconv1:c8 8*11*1->18*24*1
W_deconv1 = tf.Variable(tf.truncated_normal([4, 4, 1, 1]))
c8_2x = tf.nn.conv2d_transpose(c8_relu, W_deconv1,[BATCH_SIZE, 18, 24, 1], [1, 2, 2, 1], 'VALID')
#reduce dim: conv5 18*24*256->18*24*1
...
#deconv1:x2 18*24*1->37*49*1
W_deconv2 = tf.Variable(tf.truncated_normal([3, 3, 1, 1]))
#h = (h_in - 1) * stride_h + kernel_h - 2 * pad_h
x2_2 = tf.nn.conv2d_transpose(x2, W_deconv2,[BATCH_SIZE, 37, 49, 1], [1, 2, 2, 1], 'VALID')
#reduce dim: conv2 37*49*256->37*49*1
...
#deconv1:x2 37*49*1->300*400*1
W_deconv3 = tf.Variable(tf.truncated_normal([11, 11, 1, 1]))
x32 = tf.nn.conv2d_transpose(x4, W_deconv3,[BATCH_SIZE, 300, 400, 1], [1, 8, 8, 1], 'VALID')
...
train_step = tf.train.AdadeltaOptimizer(0.1).minimize(loss_L2)
